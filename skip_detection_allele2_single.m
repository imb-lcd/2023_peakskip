...
skip_p53=NaN(size(M2Tot));
skip_mdm2=NaN(size(M2Tot));
skip_start=NaN(size(M2Tot));
...
for i=2:length(plocs)-1
    ...
    if isempty(idx)
        ...
        skip_mdm2((path(xx5,1)):(path(xx6,1)))=M2Tot((path(xx5,1)):(path(xx6,1)));
        ...
        detection_point{2}=vertcat(detection_point{2},plocs(i));
...
plot(t,skip_start-20,'LineStyle','none','Marker','^' ...
    ,'MarkerFaceColor',c(3,:),'MarkerEdgeColor','k'...
    ,'MarkerSize',6);

nexttile(3);
hold on
plot(t,skip_mdm2,'r','LineWidth',2.5);
plot(t,skip_start,'LineStyle','none','Marker','^' ...
    ,'MarkerFaceColor',c(3,:),'MarkerEdgeColor','k'...
    ,'MarkerSize',6);